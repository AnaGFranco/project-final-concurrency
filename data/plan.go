...
// Plan é o tipo de planos de assinatura
type Plan struct {
	ID                  int
	PlanName            string
	PlanAmount          int
...
// GetOne retorna um plano por id
...
// SubscribeUserToPlan inscreve um usuário em um plano por inserção
func (p *Plan) SubscribeUserToPlan(user User, plan Plan) error {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()

	// excluir plano existente, se houver
...
// AmountForDisplay formata o preço que temos no banco de dados como uma string de moeda
